...
useEffect(() => {
    ...
    const subscription = DataStore.query(Category, Predicates.ALL).then((items) => {
        setCategories(items);
        setIsLoading(false);
    });
}, []);
...
const renderCategoryList = () => {
    return categories.map((category) => {
        return (
            <Picker.Item label={category.label} value={category.label} key={category.id}/>
        )
    })
}
...
const product = useContext(productContext);
    ...
    async function SaveOffer() {
        ...
        let timeStartString = DateTime.fromISO(dateStart.toISOString()).set({hour: timeStart.getHours(), minute: timeStart.getMinutes()}).toISO();
        let timeEndString = DateTime.fromISO(dateEnd.toISOString()).set({hour: timeEnd.getHours(), minute: timeEnd.getMinutes()}).setLocale('fr').toISO();
